...
ui <- navbarPage(inverse = TRUE,
                 ...
                 tabPanel(icon("home"), 
                          fluidPage(
                            ...
                            p(actionLink("link_to_tabpanel_STRdatasets",strong("1. STR callsets[Searchable]",style = "color:#337ab7")),style = "font-size: 14pt"),
                            ...
                            p(actionLink("link_to_tabpanel_PCA",strong("2. PCA",style = "color:#337ab7")),style = "font-size: 14pt"),
                            ...
                            p(strong("Access by clicking on each row of each dataset in"),actionLink("link_to_tabpanel_TopSTRSTR",strong("TopSTR",style = "color:#337ab7")), ".",
                              style = "font-size: 13pt"),
                            p(actionLink("link_to_tabpanel_Accessible_STR",strong("5. Genetic Crosses STR",style = "color:#337ab7")),style = "font-size: 14pt"),
                            p("Genome-wide accessible STR variation in ", em("P. falciparum "),  "genome based on ",
                              a(strong(em("P. falciparum Genetic Crosses.")),style = "color:#337ab7", href = "https://www.malariagen.net/parasite/p-falciparum-genetic-crosses"),
                              style= "font-size: 13pt"),
                            p(style="text-align: justify;","These STRs show highly accurate calls (No Mendelian errors or discordance between biological replicates) in three experimental genetic crosses (", 
                              a(strong("3D7×HB3, Walliker et al. 1987; "),style = "color:#337ab7", href = "https://www.science.org/lookup/doi/10.1126/science.3299700"),
                              a(strong("HB3×Dd2, Wellems et al. 1990; "),style = "color:#337ab7", href = "https://www.nature.com/articles/345253a0"),
                              a(strong("7G8×GB4, Hayton et al. 2008) "),style = "color:#337ab7", href = "https://www.cell.com/cell-host-microbe/fulltext/S1931-3128(08)00185-6?_returnURL=https%3A%2F%2Flinkinghub.elsevier.com%2Fretrieve%2Fpii%2FS1931312808001856%3Fshowall%3Dtrue"),
                              "of ", em("P. falciparum "), "(98 samples).",style = "font-size: 13pt"),
                            
                            
                            #Change
                            p(actionLink("link_to_tabpanel_STR_parameter",strong("6. STR model parameter",style = "color:#337ab7")),style = "font-size: 14pt"),
                            
                            p("Genome-wide high-quality STRs of ",em("P. falciparum "),"and",em("P. vivax"),"multivariable logistic regression model parameter.",
                              style= "font-size: 13pt"),
                            br(),
                            br(),
                            br(),
                 ...
                 tabPanel("PCA",
                          fluidPage(
                            ...
                            mainPanel(
                              ...
                              br(),
                              br(),
                              p(strong("PCA plot of 3,496 genome-wide high-quality STRs (Pv 174 samples)"),style = "font-size: 17pt"),
                              br(),
                              br(),
                              plotlyOutput('Pv_PCA'),
                              br(),
                 ...
                 #Pf Pf3K dataset
                 tabPanel("GeneticCross_STR",
                          fluidPage(
                            h1("Accessible STR variation in", em("Plasmodium falciparum"), "genome (P. falciparum Genetic Crosses)"),
                            sidebarLayout(position = "left",
                                          sidebarPanel(
                                            wellPanel(style = "background: white",
                                                      selectInput(inputId = "datasetRs",
                                                                  label = "Please choose a dataset:",
                                                                  choices = c("","Pf")),
                                                      actionButton("ViewR5","Generate Table"))
                                          ),
                                          
                                          mainPanel(
                                            br(),
                                            DT::dataTableOutput("viewRs")
                                          )
                            )
                          )
                 ),
                 
                 #Pf and Pv model parameter
                 tabPanel("STR_Model_Parameter",
                          fluidPage(
                            h1("STR multivariable logistic regression model parameter"),
                            sidebarLayout(position = "left",
                                          sidebarPanel(
                                            wellPanel(style = "background: white",
                                                      selectInput(inputId = "datasetMs",
                                                                  label = "Please choose a dataset:",
                                                                  choices = c("","Pf","Pv")),
                                                      actionButton("ViewM7","Generate Table"))
                                          ),
                                          
                                          mainPanel(
                                            br(),
                                            DT::dataTableOutput("viewMs")
                                          )
                            )
                          )
                 )
...
server <- function(input, output,session) {
  ...
  observeEvent(input$link_to_tabpanel_Accessible_STR,{
    newvalue <- "GeneticCross_STR"
    updateNavbarPage(session, "panels_main", newvalue)
  }) 
 ...
 # }) 
  ...
  Pf = good_pf %>% 
    dplyr::select(STRLocus,Motif,copies,Gene,Product.Description,Category,Heterozygosity,PlasmoDB_Link,Protein_Link) %>% 
    dplyr::rename("STR Locus (Pf3D7)" = "STRLocus") %>% 
    dplyr::rename("# copies (Pf3D7)" = "copies")
  Pv = good_pv %>% 
    dplyr::select(STRLocus,Motif,copies,Gene,Product.Description,Category,Heterozygosity,PlasmoDB_Link) %>% 
  ...
  db_Rshiny <- reactive({
    require(input$ViewR5)
    if (input$datasetRs == "Pf") {
      str_Rshiny
    }
  })
  ...
  data_Rshiny <- eventReactive(input$ViewR5, {
    db_Rshiny()
  })
  
  output$viewRs <- DT::renderDataTable(
    DT::datatable(data_Rshiny(),caption = 'Table: This is genome-wide high-quality STR loci table (P. falciparum Genetic Crosses).',
                  filter = 'top', 
                  extensions = 'Buttons',
                  options = list(dom = 'Blfrtip',
                                 buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
