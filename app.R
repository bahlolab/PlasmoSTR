...
#good_pv <- read.csv("milton_home/PlasmodiumSTR/Pv_STR_Top_Table.csv")
good_pv$Heterozygosity <- round(good_pv$Heterozygosity,3)
good_pf <- read.csv("Pf_STR_Top_Table.csv")
#good_pf <- read.csv("milton_home/PlasmodiumSTR/Pf_STR_Top_Table.csv")
...
#geno_pf <- read.csv("milton_home/PlasmodiumSTR/Pf_Genotype.csv")
geno_origin_Pf <- geno_pf
Pf_pop_loci <- read.csv("Pf_STR_Top_ShinyPlot.csv")
#Pf_pop_loci <- read.csv("milton_home/PlasmodiumSTR/Pf_STR_Top_ShinyPlot.csv")
Pf_pop_loci <- good_pf %>% 
...
#pop_pf_population <- read.csv("milton_home/PlasmodiumSTR/Pf_Sample.csv")
geno_pf_population <- geno_pf_population %>% 
...
#Pf_country_loci <- read.csv("milton_home/PlasmodiumSTR/Pf_STR_Top_ShinyPlot.csv")
...
#pop_pf_country <- read.csv("milton_home/PlasmodiumSTR/Pf_Sample.csv")
geno_pf_country <- geno_pf_country %>% 
...
#geno_pv <- read.csv("milton_home/PlasmodiumSTR/Pv_Genotype.csv")
geno_origin_Pv <- geno_pv
Pv_country_loci <- read.csv("Pv_STR_Top_ShinyPlot.csv")
#Pv_country_loci <- read.csv("milton_home/PlasmodiumSTR/Pv_STR_Top_ShinyPlot.csv")
...
#pop_pv_country <- read.csv("milton_home/PlasmodiumSTR/Pv_Sample.csv")
...
#PF PCA
Pf_PCA <- read.csv("PCA_Pf.csv")
Pf_PCA$Population <- factor(Pf_PCA$Population,levels = c("CAF","EAF","WAF","SAM","OCE","SAS","ESEA","WSEA"))

#Pv PCA
Pv_PCA <- read.csv("PCA_Pv.csv")

#navil panel
ui <- navbarPage(inverse = TRUE,
                 strong("PlasmodiumSTR"), 
                 theme = shinytheme("cerulean"),
                 id = "panels_main",
                 tabPanel(icon("home"), 
                          fluidPage(
                            h1("PlasmodiumSTR: A population-wide database of short tandem repeat variation in ", em("Plasmodium falciparum"), "and ", em("Plasmodium vivax")),
                            br(),
                            h3("About PlasmodiumSTR"),
                            br(),
                            p(strong("PlasmodiumSTR contains population and genome-wide information about genetic variation and other characteristics of short tandem repeats (STRs) in ",em("Plasmodium falciparum"), "and ", em("Plasmodium vivax"), " .",style = "font-size:20px;")),
                            br(),
                            ...
                            p(style="text-align: justify;","A genomic region of STR Locus using the syntax chrom:start-end (e.g., 7:1322349-1322361). PlasmodiumSTR is currently based on Pf3D7 (v3 PlasmoDB-41) and PvP01 (PlasmoDB release 41) coordinates.",style = "font-size: 13pt"),
                          
                            p(actionLink("link_to_tabpanel_TopSTR",strong("3. TopSTR",style = "color:#337ab7")),style = "font-size: 14pt"),
                            p("The top ten most highly differentiated STRs based on ", em("Jost's D "), "proposed by Jost (2008)", ".", style= "font-size: 13pt"),
                            ...
                            p(style="text-align: justify;","Click each row of the datasets can show the samples corresponding motif copies plots. The figures is shown in ", actionLink("link_to_tabpanel_TopSTRMotifPlotsPlots",strong("TopSTRMotifPlots",style = "color:#337ab7")),style = "font-size: 13pt"),
                            ...
                            p(actionLink("link_to_tabpanel_TopSTRMotifPlots",strong("4. TopSTRMotifPlots",style = "color:#337ab7")),style = "font-size: 14pt"),
                            p(style="text-align: justify;","Samples corresponding motif copies and allele frequency plots. Plot samples corresponding motif copies distribution of each highly differentiated STR locus (based on ",em("Jost's D"), "value) over different population or country pair.",style = "font-size: 13pt"),
                            p(strong("Access by clicking on each row of each dataset in"),actionLink("link_to_tabpanel_TopSTRSTR",strong("TopSTR",style = "color:#337ab7")),style = "font-size: 13pt"),
                            br(),
                            br(),
                            ...
                            br(),
                            br(),
                            div(p(strong("PlasmodiumSTR was developed by Jiru Han with input from Jacob E. Munro, Melanie Bahlo and other "), a("Bahlo Lab members", href = "https://github.com/orgs/bahlolab")), 
                                p(strong("It has been made with Rstudio and Shiny and you may find the code on "), a("github", href = "https://github.com/orgs/bahlolab"), "using the power of Rstudio and Shiny."),
                                style="text-align: right;")
                          
                            #div(p(strong("PlasmodiumSTR was developed by"), a("Jiru Han", href = "https://twitter.com/committedtotape"), "using the power of Rstudio and Shiny."), 
                            #    p(strong("R Packages:"), "tidyverse, tidytext, wordcloud2, tidygraph, vizNetwork, glue."),
                            #    p(strong("Sources:"), a("genius.com", href = "https://github.com/orgs/bahlolab/"), style="text-align: right;")
                          )
                 ),
                 ...
                 # Third Page  - Highly different STRs       
                 tabPanel("TopSTR",
                          ...
                          navlistPanel(
                            ...
                            tabPanel("1. Pf Population Pair",
                                     #tags$style(".nav-tabs {background-color: #006747;}"),
                                     fluidRow(column(12,
                 ...
                 # Four Page  - plots   Pf population   
                 tabPanel("TopSTRMotifPlots",
                          h2("Samples corresponding STR copies plots (Highly differentiated STRs of each pair population or country)"),
                          br(),
                          p("Please select the pair that you selected on ", actionLink("link_to_tabpanel_TopSTRSTRSTR",strong("TopSTR",style = "color:#337ab7")),
                            style = "font-size: 15pt"),
                          mainPanel(
                            id="Try",
                            #"Please select the panel that you click the table",
                            tabsetPanel(
                              tabPanel("Pf Population Pair",
                                       ...
                                       fluidRow(
                                         column(12,
                                                #h3("STR copies of samples in highly differentiated STR locus"),
                                               h3(textOutput('textPf')),
                                                ...
                                                #h3("Allele frequency of samples in highly differentiated STR locus"),
                              ...
                              tabPanel("Pf Country Pair",
                                       #"2. Samples corresponding genotypes plots Pf",
                                       fluidRow(column(6,
                                                       wellPanel(
                                                         style = "background: white",
...
server <- function(input, output,session) {
  ...
  observeEvent(input$link_to_tabpanel_TopSTRSTRSTR,{
    newvalue <- "TopSTR"
    updateNavbarPage(session, "panels_main", newvalue)
  }) 
  
  
  observeEvent(input$link_to_tabpanel_TopSTRMotifPlots,{
    newvalue <- "TopSTRMotifPlots"
    updateNavbarPage(session, "panels_main", newvalue)
  }) 
  
  
  observeEvent(input$link_to_tabpanel_TopSTRMotifPlotsPlots,{
    newvalue <- "TopSTRMotifPlots"
    updateNavbarPage(session, "panels_main", newvalue)
  }) 
  ...
  Pf = good_pf %>% 
    ...
    rename("STR Locus (Pf3D7)" = "STRLocus") %>% 
    rename("# copies (Pf3D7)" = "copies")
  Pv = good_pv %>% 
    dplyr::select(STRLocus,Motif,copies,Product.Description,Category,Heterozygosity) %>% 
    rename("STR Locus (PvP01)" = "STRLocus") %>% 
    rename("# copies (PvP01)" = "copies")
  ...
  observeEvent(input$view2_rows_selected,{
    ...
    selectedCellsPf <- renderText(datasetInput2()[input$view2_rows_selected,7])
    ...
    updateNavbarPage(session, "panels_main", selected = "TopSTRMotifPlots")
    #updateNavbarPage(session, "TopSTRMotifPlots", selected = "Pf Population Pair")
  }) 
 ...
 # observeEvent(input$view4_rows_selected,{
  ...
  #  output$textPv <- renderText({"Allele frequency of samples in highly differentiated STR locus"})
 ...
 # }) 
  ...
  observeEvent(input$view3_rows_selected,{
    ...
    selectedCellsPfc <- renderText(datasetInput3()[input$view3_rows_selected,7])
  ...
  observeEvent(input$view4_rows_selected,{
    ...
    selectedCells <- renderText(datasetInput4()[input$view4_rows_selected,7])
